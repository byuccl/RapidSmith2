...
Section \ref{sec:vdiImport} introduces RSCPs and \autoref{sec:vdiExport} introduces
...
technically redundant, in that is also included in the EDIF netlist (described
in the next subsection), but can be easier to parse than its EDIF equivalent. The
...
Figure \ref{fig:routethroughs2} shows two example routethrough LUTs in Vivado.
...
configuration equation for this LUT is slightly different. As figure
\ref{fig:routethroughs2} shows, VCC is routed to the \texttt{A6} input pin. In
this case, the configuration equation on the BEL is
...
reasons discussed in \autoref{sec:tclChallenges}, device PIPs are the best option
...
\autoref{sec:vdiImport}. The \texttt{Tincr} function used to parse TCPs,
...
XDC, are described more in \autoref{sec:tclChallenges}
...
As described in \autoref{sec:routingXdc}, the \texttt{ROUTE} property
...
As discussed in \autoref{sec:alternateSites}, the Vivado Tcl command
...
As described in \autoref{sec:macros} fully-flattened Vivado netlists may
...
As described in \autoref{sec:routingRsc}, the \texttt{CONFIG.EQN} property on a
...
tools \textit{and} Vivado''? The purpose of this section is to give insight
into four reasons why both RSCPs and TCPs are required with VDI. 
...
\begin{enumerate}
  ...
  \texttt{ROUTE} strings as described in \autoref{sec:routeStrings}, they are
  ...
  (\autoref{sec:routingXdc}). This indicates that a different format is needed
