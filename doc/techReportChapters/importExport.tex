...
% 	- How to load a RSCP into RS2 
%	- The contents of Tincr Checkpoints (TCP)
% 	- How to export a TCP from RS2 
...
post-place, and post-route), and can be parsed and imported into RS2. As
...
command \texttt{write\_edif}. A RS2 \celldesign is created by parsing the
...
to execute before generating a bitstream. When importing a RSCP into RS2,
...
demonstrates how to manipulate XDC constraints in RS2. Future RS2 plans
...
RS2 can be found in \autoref{sec:macros}.
...
RS2. The main different between TCPs and RSCPs, is that the
...
information to the design. On design export, RS2 produces
...
TCP from RS2 and view the individual files.
...
represented. In order to accurately represent a design in RS2, they
must be included somehow. This section describes the parts of the design that
aren't explicitly represented in Vivado, and how RS2 handles them.
...
RS2 from every input LUT pin to the LUTs output pin. These \bel
...
routethrough in RS2, you simply have to include the corresponding
\cls{WireConnection} within an INTRASITE \cls{RouteTree}. When a design is
exported from RS2, the routethrough \cls{WireConnection}s are
...
\texttt{::tincr::write\_rscp} command, it can be loaded into RS2.
...
\noindent
While a design is being imported into RS2, several useful data structures
...
\noindent
After a design is exported from RS2, a Tincr Checkpoint (TCP) is
