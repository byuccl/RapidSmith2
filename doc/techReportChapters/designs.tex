...
for Ultrascale devices. It is suggested that users of RS2 become
...
recreate the design in RS2, the EDIF file is parsed and converted to a
custom netlist structure described in the next section.

\subsection {RS2 Netlist Data Structures} \label{sec:designDS}

RS2 netlists are modeled closely after Xilinx netlists. In fact,
...
RS2 straightforward. The data structures that constitute a RS2
...
\begin{figure}[H]
 ...
 \caption{RS2 design data structure tree. Black arrows represent
...
\noindent
A \cls{CellDesign} is the top level design object in RS2. It consists of
a collection of \cls{Cell} objects, interconnected by \cls{CellNet}s. RS2
\cls{Cell}s are equivalent to Xilinx \cls{Cell}s and RS2 \cls{CellNet}s
...
structure. A Vivado design is converted from a RapidSmith Checkpoint to a
...
\begin {itemize}
  ...
  \autoref{code:cells} demonstrates how to create a new cell in RS2 and
  filter cells based on their type.
  
\begin{lstlisting}[caption=How to create new cells in RS2,
label=code:cells] 
...
\end{lstlisting}
  ...
  Port \cls{Cell}s in RS2. Specifically, there are three types of port cells: IPORT, 
...
There are two types of \cls{CellPin} objects in RS2:
...
\begin{figure}[H]
 ...
 \caption{An example where a \cls{PseudoCellPin} in RS2 may be needed.
...
In RS2, a \cls{PseudoCellPin} can be added to the LUT2 cell (called
...
automatically to \cls{Cell}s when importing a RapidSmith Checkpoint. If you are
...
important aspects about \cls{CellNet}s in RS2.

\begin{itemize}
  \item \cls{CellNet}s are routed using the \cls{RouteTree} data structure.
  Routing in RS2 is described in more detail in \autoref{sec:routing}.
  ...
  RS2 in two ways. (1) Vivado designs can have multiple nets that
  ...
  RS2 takes a different approach. When a design is imported from Vivado,
  ...
  RS2 handles this oddity by allowing \cls{CellNet}s to have more than
  ...
  external source. RS2 supports nets with multiple drivers. The
...
Most cells in RS2 or Vivado designs are leaf cells (LUTs, Flip Flops,
...
internal cells together. RS2 now supports importing macro cells from
Vivado and adding them to a \cls{CellDesign}. \autoref{code:macros} gives a
brief introduction to using macro in RS2.

\begin{lstlisting}[caption=How to use macros in RS2,
label=code:macros] 
...
\begin{itemize}
  ...
  \item When a macro cell pin is connected to a \cellnet, RS2
  ...
  disconnected. Nets in RS2 only connect to leaf cells (i.e. it is
  ...
  placed instead.
  \item When a \cls{CellDesign} is exported from RS2, macro cells are not
...
included in the output EDIF netlist of a RapidSmith Checkpoint (\pgm{if they are
not the default value}).

When RS2 parses the EDIF file in a RapidSmith Checkpoint, the properties
within are stored in a data structure called a \cls{PropertyList}. Each
\cls{CellDesign}, \cls{Cell}, and \cls{CellNet} in RS2 has an associated
...
\begin{lstlisting}[caption=Using PropertyLists in RS2,
...
\begin{itemize}
  ...
  \pgm{RS2 makes no attempt to parse the Vivado properties into their
  ...
  but they will not be listed here. When creating designs in RS2, it is
  important to not place \cls{Cell}s together that may have conflicting
  properties. RS2 does not perform any error checking, and so an error
...
The \cls{CellLibrary} in RS2 stores important information about each of
...
part.
This means that for each part you use in RS2, a new \cls{CellLibrary}
...
RS2, and how to create a new cell library XML file from Tincr.

\subsubsection{Loading a Cell Library}
When a RapidSmith Checkpoint is imported into RS2, a \cls{CellLibrary} is
automatically loaded. To get a handle to the \cls{CellLibrary}, you can use
the method call \texttt{VivadoCheckpoint.getLibCells()} on the loaded checkpoint
object. If you don't have a handle to a RapidSmith Checkpoint, then the cell
...
// First way, load a RapidSmith Checkpoint
