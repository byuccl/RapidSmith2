...
public class TileWindowJavaFx extends Canvas {
    /**DeviceBrowser copy*/
...
//        initializeScene(true, true);
     ...
     */
    public TileWindowJavaFx(DeviceBrowserJavaFx dB, Device device, boolean hideTiles, boolean drawSites){
    ...
    private void drawFPGAFabric(boolean drawSites){
        gc.clearRect(0,0,(cols + 1) * (tileSize + 1), (rows + 1) * (tileSize + 1));
        gc.setFill(Color.BLACK);
        gc.fillRect(0, 0, (cols + 1) * (tileSize + 1), (rows + 1) * (tileSize + 1));//might not need
        ...
        for(int col : colsToSkip){
            int realCol = col - i;
            gc.strokeLine(tileSize*realCol-1, 0, tileSize*realCol-1, rows*tileSize-3);
            i++;
        }
        i=0;
        for(int row : rowsToSkip){
            int realRow = row - i;
            gc.strokeLine(0,tileSize*realRow-1, cols*tileSize-3,tileSize*realRow-1);
            i++;
        }
//
