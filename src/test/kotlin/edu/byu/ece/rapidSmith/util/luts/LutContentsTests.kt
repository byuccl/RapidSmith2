...
class LutContentsTests {
	...
	class ParseEquationTests {
		...
		@TestFactory
		@DisplayName("test single input")
		fun testSingleInput(): List<DynamicTest> {
			return (1..6).map { DynamicTest.dynamicTest("A$it", {
				val given = LutContents.parseEquation("when =A$it")
				val equation = given.equation
				val expected = LutInput(it)
				assertEquals(expected.toString(), equation.toString())
			}) }
		}

		@TestFactory
		@DisplayName("test binary equation")
		fun testBinaryEquation(): List<DynamicTest> {
			return listOf("*", "+", "@").map { DynamicTest.dynamicTest("when =A1${it}A2", {
				val given = LutContents.parseEquation("A1${it}A2")
				val equation = given.equation
				val expected = BinaryOperation(OpType.fromSymbol(it[0]), LutInput(1), LutInput(2))
				assertEquals(expected.toString(), equation.toString())
			}) }
		}

		@Test
		@DisplayName("test single parens")
		fun testSingleParens() {
			val given = LutContents.parseEquation("when =(A1*A2)")
			val equation = given.equation
			val expected = BinaryOperation(OpType.AND, LutInput(1), LutInput(2))
			assertEquals(expected.toString(), equation.toString())
		}

		@Test
		@DisplayName("test inverted input")
		fun testInvertedInput() {
			val given = LutContents.parseEquation("when =~A1")
			val equation = given.equation
			val expected = LutInput(1, true)
			assertEquals(expected.toString(), equation.toString())
		}
		...
		fun testUnusedA6PinEquation() {
			val given = LutContents.parseEquation("when =((A6+~A6)*A5)")
			val equation = given.equation
			val left = LutInput(5)
