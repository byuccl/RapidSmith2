...
public final class VivadoInterface {
	...
	private static HashSet<String> usedPips = new HashSet<>();
	private static HashMap<String, RouteTree> pinToRT = new HashMap<>();
	private static HashMap<String, HashMap<String, RouteTree>> siteToRTs = new HashMap<>();

	//list which holds the cells in the order they were read into Vivado
	private static ArrayList<Cell> cellsInOrder = new ArrayList<>();
		...
		while ((line = br.readLine()) != null) {
			...
			//handle intersite (between sites) routing
				...
				if(net.getSourcePin() == null) {
					...
					if(net.isClkNet()) {
						//TODO: Add functionality for clock nets
					}
	...
	private static RouteTree initializeRouteTree(CellNet net, String firstWireName) {
		...
		RouteTree start = new RouteTree(firstWire);
